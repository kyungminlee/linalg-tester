...
### Reference/system BLAS — double precision

The `examples/reference_blas/` directory tests the system `libblas.so.3`,
which is the standard Fortran-77 BLAS interface provided by the system's BLAS
alternative (Netlib reference BLAS, OpenBLAS, ATLAS, MKL, …).
...
The script adds the library's parent directory to `LD_LIBRARY_PATH` so any
transitively-needed libraries (e.g. `libopenblas.so.0`) are resolved correctly,
and auto-builds a stub `libgfortran.so.5` if the real one is not installed.

**Sample output** (libblas.so.3 → OpenBLAS 0.3.26, m=n=k=64, seed=42, prec=256 bits):
