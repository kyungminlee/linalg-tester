/* ptrmv.cpp -- PBLAS Level 2 PTRMV accuracy tester */
...
/* Fortran-ABI function pointer for PDTRMV (hidden char lengths at end) */
...
/* ------------------------------------------------------------------ */
/* MPFR reference: x_ref = op(A) * x_in  (in-place on vector)         */
/* ------------------------------------------------------------------ */

static void mpfr_ptrmv_ref(MpfrMatrix &x_ref,
                            ...
                            const MpfrMatrix &x_in)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int p = 0; p < n; ++p)
            mpfr_fma(acc.get(), Aw.at(i, p), x_in.at(p, 0), acc.get(), MPFR_RNDN);
                ...
                const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PTRMV", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<ptrmv_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PTRMV", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char diag : {'N', 'U'}) {
        ...
        unsigned sentinel_seed = 0xFBAD0009u;
        ...
        /* MPFR reference on saved global x */
        MpfrMatrix A_mpfr(n, n, prec);
        MpfrMatrix x_in_mpfr(n, 1, prec);
        MpfrMatrix x_ref(n, 1, prec);

        custom_to_mpfr_mat(A_mpfr, A_g, n, ctx);
        custom_to_mpfr_mat(x_in_mpfr, x_g, n, ctx);

        mpfr_ptrmv_ref(x_ref, uplo, trans, diag, n, A_mpfr, x_in_mpfr);
        ...
        if (pc.bc.is_root()) {
            ...
            report_result("PTRMV", params_str, err, &sr, format);
        }
