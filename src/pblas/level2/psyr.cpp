/* psyr.cpp -- PBLAS Level 2 PSYR accuracy tester */
...
/* Fortran-ABI function pointer for PDSYR (hidden char length at end) */
...
/* ------------------------------------------------------------------ */
/* MPFR reference: A_ref = alpha * x * x^T + A_in (full n x n)        */
/* ------------------------------------------------------------------ */

static void mpfr_psyr_ref(MpfrMatrix &A_ref,
                           int n,
                           mpfr_t alpha,
                           const MpfrMatrix &x,
               ...
               const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PSYR", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<psyr_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PSYR", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        unsigned sentinel_seed = 0xFBAD000Cu;
        ...
        mpfr_psyr_ref(A_ref, n, mpfr_alpha.get(), x_mpfr, A_in_mpfr);
        ...
        if (pc.bc.is_root()) {
            ...
            report_result("PSYR", params_str, err, &sr, format);
        }
