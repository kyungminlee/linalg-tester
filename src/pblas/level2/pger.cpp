/* pger.cpp -- PBLAS Level 2 PGER accuracy tester */
...
/* Fortran-ABI function pointer for PDGER (no hidden char lengths) */
...
static void mpfr_pger_ref(MpfrMatrix &A_ref,
                           int m, int n,
                           mpfr_t alpha,
                           const MpfrMatrix &x,
                           const MpfrMatrix &y,
               ...
               const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PGER", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<pger_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PGER", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
...

    {
        ...
        void *x_g   = gen_random_array(m, ctx.typesize, ctx.from_mpfr, prec, &seed_x);
        ...
        unsigned sentinel_seed = 0xFBAD000Bu;
        ...
        custom_to_mpfr_mat(x_mpfr, x_g, m, ctx);
        custom_to_mpfr_mat(y_mpfr, y_g, n, ctx);
        custom_to_mpfr_mat(A_in_mpfr, A_g, m, ctx);

        mpfr_pger_ref(A_ref, m, n,
                       mpfr_alpha.get(), x_mpfr, y_mpfr, A_in_mpfr);

        /* Extract local reference and compare */
        MpfrMatrix A_local_ref(loc_rA, loc_cA, prec);
        extract_local_mpfr(A_local_ref, A_ref, m, n, pc.mb, pc.nb,
                           pc.bc.myrow, pc.bc.mycol, pc.bc.nprow, pc.bc.npcol);
        ...
        if (pc.bc.is_root()) {
            ...
            report_result("PGER", params_str, err, &sr, format);
        }
