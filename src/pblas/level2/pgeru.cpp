/* pgeru.cpp -- PBLAS Level 2 PGERU accuracy tester (complex-only) */
...
/* Fortran-ABI function pointer for PCGERU/PZGERU (no char params, no hidden lengths) */
...
/* ------------------------------------------------------------------ */
/* MPFR reference: A_ref = alpha * x * y^T + A_in  (no conjugation)    */
/* ------------------------------------------------------------------ */

static void mpfr_pgeru_ref(MpfrComplexMatrix &A_ref,
                            int m, int n,
...
{
    if (!ctx.complex_mode) {
        std::fprintf(stderr, "PGERU requires --complex\n");
        return;
    }
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PGERU", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<pgeru_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PGERU", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
...

    {
        ...
        unsigned sentinel_seed = 0xFBAD001Fu;
        ...
        mpfr_pgeru_ref(A_ref, m, n, mpfr_alpha, x_mpfr, y_mpfr, A_in_mpfr);
        ...
        if (pc.bc.is_root()) {
            ...
            report_result("PGERU", params_str, err, &sr, format);
        }
