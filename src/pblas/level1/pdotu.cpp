/* pdotu.cpp -- PBLAS Level 1 PDOTU accuracy tester (complex unconjugated dot) */
...
/* Fortran-ABI function pointer for PZDOTU */
extern "C" typedef void (*pdotu_fn_t)(
    const int  *n,
    void       *dotu,
...
{
    if (!ctx.complex_mode) {
        std::fprintf(stderr, "PDOTU requires --complex\n");
        return;
    }
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PDOTU", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<pdotu_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PDOTU", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Call PBLAS: dotu = x^T * y (no conjugation) */
    fn(&n, result_buf,
       x_loc, &one, &one, desc_x, &incx,
       y_loc, &one, &one, desc_y, &incy);

    /* MPFR reference: dotu_ref = sum of x[i] * y[i] (unconjugated) */
    ...
    MpfrComplexScalar dotu_ref(prec);
    mpfr_set_d(dotu_ref.re(), 0.0, MPFR_RNDN);
    mpfr_set_d(dotu_ref.im(), 0.0, MPFR_RNDN);
    for (int i = 0; i < n; ++i) {
        mpfr_complex_fma(dotu_ref.re(), dotu_ref.im(),
                         x_mpfr.re(i, 0), x_mpfr.im(i, 0),
                         y_mpfr.re(i, 0), y_mpfr.im(i, 0), MPFR_RNDN);
    }

    /* Compare complex scalar result */
    ErrorResult err = compute_error_complex_scalar(dotu_ref, result_buf, ctx);

    if (pc.bc.is_root()) {
        char params_str[128];
        std::snprintf(params_str, sizeof(params_str),
                      "n=%d grid=%dx%d", n, pc.bc.nprow, pc.bc.npcol);
        report_result("PDOTU", params_str, err, format);
    }
