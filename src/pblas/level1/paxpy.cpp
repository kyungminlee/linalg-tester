/* paxpy.cpp -- PBLAS Level 1 PAXPY accuracy tester */
...
/* Fortran-ABI function pointer for PDAXPY (no hidden char lengths) */
                ...
                const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PAXPY", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<paxpy_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PAXPY", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Generate global vectors and alpha */
    ...
    unsigned sentinel_seed = 0xFBAD0017u;
    ...
    /* Call PBLAS: y = alpha * x + y */
    fn(&n, alpha,
       x_loc, &one, &one, desc_x, &incx,
       y_loc, &one, &one, desc_y, &incy);

    /* MPFR reference: y_ref[i] = fma(alpha, x[i], y[i]) */
    ...
    for (int i = 0; i < n; ++i)
        mpfr_fma(y_ref.at(i, 0), mpfr_alpha.get(), x_mpfr.at(i, 0),
                 y_mpfr.at(i, 0), MPFR_RNDN);
    ...
    if (pc.bc.is_root()) {
        ...
        report_result("PAXPY", params_str, err, &sr, format);
    }
