/* ptranu.cpp -- PBLAS Level 3 PZTRANU accuracy tester (complex unconjugated transpose) */
...
/* Fortran-ABI function pointer for PZTRANU (no char args, no hidden lengths) */
extern "C" typedef void (*ptranu_fn_t)(
...
/*   A is m-by-n, C is n-by-m  (no conjugation -- unconjugated transpose) */
/* ------------------------------------------------------------------ */

static void mpfr_ptranu_ref(MpfrComplexMatrix &C_ref,
                              int m, int n,
                              ...
                              const MpfrComplexMatrix &C_in)
{
    ...
    MpfrComplexScalar alpha_term(prec), beta_term(prec);
...
{
    if (!ctx.complex_mode) {
        fprintf(stderr, "PTRANU requires --complex\n");
        return;
    }
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PTRANU", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<ptranu_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PTRANU", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
...

    {
        ...
        unsigned sentinel_seed = 0xFBAD0025u;
        ...
        if (pc.bc.is_root()) {
            ...
            report_result("PTRANU", params_str, err, &sr, format);
        }
