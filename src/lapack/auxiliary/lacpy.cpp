/* lacpy.cpp -- LAPACK LACPY accuracy tester (matrix copy) */
    ...
    for (char uplo : {'U', 'L', 'A'}) {
        ...
        for (int j = 0; j < n; ++j) {
            for (int i = 0; i < m; ++i) {
                ...
                if (copy_elem)
                    mpfr_set(B_ref.at(i, j), A_mpfr.at(i, j), MPFR_RNDN);
                else
                    mpfr_set_d(B_ref.at(i, j), 0.0, MPFR_RNDN);
            }
        }

        ErrorResult err = compute_error_matrix(B_ref, B, ldb, ctx);

        char params_str[128];
        std::snprintf(params_str, sizeof(params_str),
                      "uplo=%c m=%d n=%d", uplo, m, n);
        report_result("LACPY", params_str, err, format);
