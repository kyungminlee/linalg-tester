/* clansy.cpp -- LAPACK CLANSY/ZLANSY accuracy tester (complex symmetric matrix norm) */
...
/* ------------------------------------------------------------------ */
/* Helper: expand complex symmetric matrix from one triangle            */
/* A(i,j) = A(j,i) -- no conjugation, unlike Hermitian                 */
/* ------------------------------------------------------------------ */

static void mpfr_expand_complex_symmetric(MpfrComplexMatrix &full,
        ...
        for (int i = 0; i < n; ++i) {
            if (uplo == 'U') {
                if (i <= j) {
                    ...
                    /* full(i,j) = A(j,i) -- plain copy, no conjugation */
                    mpfr_set(full.re(i, j), A.re(j, i), MPFR_RNDN);
                    mpfr_set(full.im(i, j), A.im(j, i), MPFR_RNDN);
                }
            } else {
                if (i >= j) {
                    mpfr_set(full.re(i, j), A.re(i, j), MPFR_RNDN);
...
/* ------------------------------------------------------------------ */
/* Complex symmetric 1-norm: exploits A(i,j) = A(j,i) => |a_ij|=|a_ji|*/
/* ------------------------------------------------------------------ */
                 ...
                 const TestParams &params, const std::string &format)
{
    ...
    for (char uplo : {'U', 'L'}) {
        for (char norm_type : {'M', '1', 'I', 'F'}) {
            ...
            void *A = gen_complex_symmetric_array(n, uplo, ctx.typesize,
            ...
            if (norm_type == '1' || norm_type == 'I') {
                ...
                ref_val = mpfr_complex_symmat_norm1(A_mpfr, uplo);
            } else {
                /* Expand symmetric matrix to full for M and F norms */
                MpfrComplexMatrix full(n, n, prec);
                mpfr_expand_complex_symmetric(full, A_mpfr, uplo);
                if (norm_type == 'M')
            ...
            /* Call the library routine -- returns a REAL scalar value.
               ZLANSY interface: double ZLANSY(norm, uplo, n, A, lda, rwork, norm_len, uplo_len) */
            ...
            if (real_typesize == 4) {
                ...
                std::fprintf(stderr, "CLANSY: unsupported real_typesize %zu, skipping\n",
                             real_typesize);
                std::free(A);
            ...
            report_result("CLANSY", params_str, err, format);
