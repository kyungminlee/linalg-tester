/* claswp.cpp -- LAPACK CLASWP/ZLASWP accuracy tester (complex row interchange) */
                 ...
                 const TestParams &params, const std::string &format)
{
    ...
    mpfr_complex_apply_ipiv_rows(A_mpfr, ipiv, m, true);

    /* Call the library routine */
    fn(&n, A, &lda, &k1, &k2, ipiv, &incx);

    ErrorResult err = compute_error_complex_matrix(A_mpfr, A, lda, ctx);

    char params_str[128];
    std::snprintf(params_str, sizeof(params_str),
                  "m=%d n=%d k1=%d k2=%d", m, n, k1, k2);
    report_result("CLASWP", params_str, err, format);
