/* cgetrs.cpp -- LAPACK CGETRS/ZGETRS accuracy tester (complex solve using LU factors) */
                 ...
                 const TestParams &params, const std::string &format)
{
    ...
    for (char trans : {'N', 'C'}) {
        ...
        if (info != 0) {
            ...
            report_result("CGETRS", params_str, err_zero, format);
        ...
        custom_to_mpfr_complex_mat(X_mpfr, B_out, ldb, ctx);

        /* Compute residual: ||A*X - B|| / (||A|| * ||X|| * n * eps)
           or ||A^H*X - B|| / (||A|| * ||X|| * n * eps) for trans='C' */
        double eps = get_eps(ctx);
        double residual;

        if (trans == 'N') {
            residual = mpfr_complex_solve_residual(A_mpfr, X_mpfr, B_mpfr, eps);
        } else {
            /* For A^H * X = B, compute ||A^H * X - B|| */
            MpfrComplexMatrix Ah(n, n, prec);
            mpfr_complex_mat_adjoint(Ah, A_mpfr);
            residual = mpfr_complex_solve_residual(Ah, X_mpfr, B_mpfr, eps);
        }
        ...
        report_lapack_result("CGETRS", params_str, lr, format);
