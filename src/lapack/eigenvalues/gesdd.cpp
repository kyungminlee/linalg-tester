/* gesdd.cpp -- LAPACK GESDD accuracy tester (SVD, divide & conquer) */
...
extern "C" typedef void (*gesdd_fn_t)(
    const char *jobz,
...
{
    auto *fn = reinterpret_cast<gesdd_fn_t>(load_sym(lib, sym));
    ...
    fn(&jobz, &m, &n, A_buf, &lda, S, U, &ldu, VT, &ldvt,
       work_q, &lwork_q, iwork, &info, (std::size_t)1);
    ...
    fn(&jobz, &m, &n, A_buf, &lda, S, U, &ldu, VT, &ldvt,
       work, &lwork, iwork, &info, (std::size_t)1);
    std::free(work);

    if (info != 0) {
        LapackResult lr = {0.0, -1.0, info};
        report_lapack_result("GESDD", "", lr, format);
    } else {
