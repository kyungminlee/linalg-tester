/* cgesdd.cpp -- LAPACK complex GESDD accuracy tester (SVD, divide & conquer) */
...
{
    auto *fn = reinterpret_cast<cgesdd_fn_t>(load_sym(lib, sym));
    ...
    /* rwork size for ZGESDD with jobz='A': max(1, 5*mn*mn + 5*mn) */
    std::size_t rwork_size = static_cast<std::size_t>(std::max(1, 5 * mn * mn + 5 * mn));
    void *rwork = std::malloc(rwork_size * real_typesize);
    ...
    fn(&jobz, &m, &n, A_buf, &lda, S, U, &ldu, VT, &ldvt,
       work_q, &lwork_q, rwork, iwork, &info, (std::size_t)1);
    ...
    fn(&jobz, &m, &n, A_buf, &lda, S, U, &ldu, VT, &ldvt,
       work, &lwork, rwork, iwork, &info, (std::size_t)1);
    std::free(work);

    if (info != 0) {
        LapackResult lr = {0.0, -1.0, info};
        report_lapack_result("CGESDD", "", lr, format);
    } else {
