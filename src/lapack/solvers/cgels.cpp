/* cgels.cpp -- LAPACK CGELS/ZGELS accuracy tester (complex least squares solve) */
...
/* ------------------------------------------------------------------ */
/* Test CGELS for a specific configuration                              */
/* ------------------------------------------------------------------ */

static void test_cgels_case(const TesterCtx &ctx, void *lib, const char *sym,
                            ...
                            char trans, int m, int n, const char *case_label)
{
    ...
    /* Generate random complex B: max(m,n)-by-nrhs, but only first m rows matter for input */
    ...
    /* Allocate workspace and call CGELS/ZGELS */
    ...
    /* For overdetermined (m >= n): least squares, check normal equations
       A^H * (A*X - B) should be zero: ||A^H*r||_1 / (||A||_1 * ||r||_1 * n * eps)
    ...
    if (m >= n) {
        /* Overdetermined: check normal equations A^H * r = 0 */
        ...
        if (norm_A > 0.0 && norm_R > 0.0)
            ...
            residual = norm_AhR / (norm_R * n * eps);
    } else {
    ...
    report_lapack_result("CGELS", params_str, lr, format);
                ...
                const TestParams &params, const std::string &format)
{
    /* Overdetermined case: m > n, trans='N' */
    {
        ...
        test_cgels_case(ctx, lib, sym, params, format, 'N', m, n, "overdetermined");
    }
    ...
    /* Underdetermined case: m < n, trans='N' */
    {
        ...
        test_cgels_case(ctx, lib, sym, params, format, 'N', m, n, "underdetermined");
    }
}
