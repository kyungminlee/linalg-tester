/* hesv.cpp -- LAPACK CHESV/ZHESV accuracy tester (Hermitian indefinite solve) */
...
/* ------------------------------------------------------------------ */
/* Test HESV for a single uplo value                                    */
/* ------------------------------------------------------------------ */

static void test_hesv_uplo(const TesterCtx &ctx, void *lib, const char *sym,
                           ...
                           char uplo)
{
    ...
    /* Generate Hermitian matrix A (returned with ld = n) */
    ...
    /* Add diagonal dominance: A(i,i) += n (real part only, diagonal of Hermitian is real) */
    {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            /* Keep imaginary part zero for diagonal (Hermitian property) */
            mpfr_set_d(diag_val.im(), 0.0, MPFR_RNDN);
            ctx.from_mpfr_complex(ap + IDX(i, i, lda) * ctx.typesize,
                                  diag_val.re(), diag_val.im(), MPFR_RNDN);
        }
    }

    /* Save A_orig in MPFR -- build full Hermitian matrix for residual */
    MpfrComplexMatrix A_orig(n, n, prec);
    custom_to_mpfr_complex_mat(A_orig, A, lda, ctx);
    /* Fill in the other triangle: A(i,j) = conj(A(j,i)) for the mirrored triangle */
    for (int j = 0; j < n; ++j) {
        for (int i = 0; i < n; ++i) {
            if (uplo == 'U' && i > j) {
    ...
    /* Allocate workspace and call CHESV/ZHESV */
    ...
    report_lapack_result("HESV", params_str, lr, format);
...
void test_hesv(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    test_hesv_uplo(ctx, lib, sym, params, format, 'U');
    test_hesv_uplo(ctx, lib, sym, params, format, 'L');
}
