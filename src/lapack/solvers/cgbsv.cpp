/* cgbsv.cpp -- LAPACK CGBSV/ZGBSV accuracy tester (complex banded linear solve) */
                ...
                const TestParams &params, const std::string &format)
{
    ...
    /* Generate a full dense complex n-by-n random matrix, then extract band entries */
    void *A_full = gen_random_complex_array(static_cast<std::size_t>(n) * n,
    ...
    /* Add diagonal dominance: add n to the real part of diagonal */
    {
        MpfrScalar nval(prec);
        mpfr_set_d(nval.get(), static_cast<double>(n), MPFR_RNDN);
        for (int i = 0; i < n; ++i)
            mpfr_add(A_orig.re(i, i), A_orig.re(i, i), nval.get(), MPFR_RNDN);
    }
    ...
    void *AB = std::calloc(static_cast<std::size_t>(ldab) * n, ctx.typesize);
    {
        ...
        for (int j = 0; j < n; ++j) {
            for (int i = std::max(0, j - ku); i <= std::min(n - 1, j + kl); ++i) {
                int row = kl + ku + i - j;
                /* Convert from MPFR complex to custom format */
                ctx.from_mpfr_complex(abp + IDX(row, j, ldab) * ctx.typesize,
    ...
    /* Call CGBSV/ZGBSV: void xGBSV(int *n, int *kl, int *ku, int *nrhs,
    ...
    report_lapack_result("CGBSV", params_str, lr, format);
