/* cpotrf.cpp -- LAPACK complex POTRF accuracy tester (Cholesky factorization) */
                 ...
                 const TestParams &params, const std::string &format)
{
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        /* Generate Hermitian positive definite matrix (n-by-n, ld=n) */
        void *A_hpd = gen_hermitian_positive_definite_array(n, ctx.typesize,
                                                             ctx.from_mpfr_complex, ctx.to_mpfr_complex,
                                                             prec, &seed_A);
        ...
        mpfr_complex_extract_triangle(A_fact, T, uplo);

        /* Compute adjoint (conjugate transpose) */
        MpfrComplexMatrix Th(n, n, prec);
        mpfr_complex_mat_adjoint(Th, T);

        /* For 'L': A = L * L^H;  for 'U': A = U^H * U */
        MpfrComplexMatrix Recon(n, n, prec);
        if (uplo == 'L')
            mpfr_complex_mat_mul_simple(Recon, T, Th);
        else
            mpfr_complex_mat_mul_simple(Recon, Th, T);

        MpfrComplexMatrix Resid(n, n, prec);
        mpfr_complex_mat_sub(Resid, A_orig, Recon);
        ...
        report_lapack_result("CPOTRF", params_str, lr, format);
