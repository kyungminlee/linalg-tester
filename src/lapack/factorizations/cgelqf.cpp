/* cgelqf.cpp -- LAPACK complex GELQF accuracy tester (LQ factorization) */
                 ...
                 const TestParams &params, const std::string &format)
{
    ...
    /* Extract L (lower triangular part of A_fact, m-by-n) */
    MpfrComplexMatrix L(m, n, prec);
    for (int j = 0; j < n; ++j)
        for (int i = 0; i < m; ++i) {
            if (i >= j) {
                mpfr_set(L.re(i, j), A_fact.re(i, j), MPFR_RNDN);
                mpfr_set(L.im(i, j), A_fact.im(i, j), MPFR_RNDN);
            } else {
    ...
    /* Reconstruct Q (n-by-n) from complex Householder reflectors stored in rows */
    MpfrComplexMatrix Q(n, n, prec);
    mpfr_complex_accumulate_Q_from_LQ(Q, A_fact, tau_mpfr, m, n);

    /* Compute L*Q */
    MpfrComplexMatrix LQ(m, n, prec);
    mpfr_complex_mat_mul_simple(LQ, L, Q);

    /* Residual: ||A - L*Q||_1 / (||A||_1 * max(m,n) * eps) */
    MpfrComplexMatrix Resid(m, n, prec);
    mpfr_complex_mat_sub(Resid, A_orig, LQ);
    ...
    report_lapack_result("CGELQF", params_str, lr, format);
