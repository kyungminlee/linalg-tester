/* cpgetrs.cpp -- ScaLAPACK CPGETRS (complex PGETRS) accuracy tester */
...
void test_cpgetrs(const TesterCtx &ctx, void *lib, const char *sym,
                  const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("CPGETRS", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPGETRS", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }

    /* Derive PZGETRF symbol: replace "getrs" with "getrf" */
    ...
    if (!fn_trf) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPGETRS", "error=getrf_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Generate global complex matrices */
    ...
    if (pc.bc.is_root()) {
        ...
        report_lapack_result("CPGETRS", params_str, lr, format);
    }
