/* placpy.cpp -- ScaLAPACK PDLACPY accuracy tester */
...
void test_placpy(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PLACPY", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PLACPY", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Call PDLACPY with uplo='A' (copy all) */
    char uplo = 'A';
    int one = 1;
    fn(&uplo, &m, &n, A_loc, &one, &one, desc_a,
       B_loc, &one, &one, desc_b, (std::size_t)1);

    /* Reference: A_loc itself is the expected result.
       Build MPFR from global A, extract local, compare with B_loc */
    MpfrMatrix A_mpfr(m, n, prec);
    custom_to_mpfr_mat(A_mpfr, A_g, m, ctx);

    MpfrMatrix A_local_ref(loc_m, loc_n, prec);
    extract_local_mpfr(A_local_ref, A_mpfr, m, n, pc.mb, pc.nb,
                       pc.bc.myrow, pc.bc.mycol, pc.bc.nprow, pc.bc.npcol);

    ErrorResult err = compute_error_matrix(A_local_ref, B_loc, lld_b, ctx);
    ...
    if (pc.bc.is_root()) {
        ...
        report_lapack_result("PLACPY", params_str, lr, format);
    }
