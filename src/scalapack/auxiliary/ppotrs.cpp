/* ppotrs.cpp -- ScaLAPACK PDPOTRS accuracy tester */
...
void test_ppotrs(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PPOTRS", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PPOTRS", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }

    /* Derive PDPOTRF symbol */
    ...
    if (!fn_trf) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PPOTRS", "error=potrf_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        if (pc.bc.is_root()) {
            ...
            report_lapack_result("PPOTRS", params_str, lr, format);
        }
