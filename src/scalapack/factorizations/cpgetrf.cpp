/* cpgetrf.cpp -- ScaLAPACK CPGETRF accuracy tester
   (Complex LU factorization with partial pivoting, verified via solve) */
...
void test_cpgetrf(const TesterCtx &ctx, void *lib, const char *sym,
                  const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("CPGETRF", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPGETRF", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (!fn_getrs) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPGETRF", "error=pgetrs_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (pc.bc.is_root()) {
        ...
        report_lapack_result("CPGETRF", params_str, lr, format);
    }
