/* pgeqrf.cpp -- ScaLAPACK PGEQRF accuracy tester
   (QR factorization, verified via ||A - Q*R|| residual) */
...
extern "C" typedef void (*porgqr_fn_t)(
...
void test_pgeqrf(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PGEQRF", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PGEQRF", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }

    /* Derive PDORGQR symbol (e.g. "pdgeqrf_" -> "pdorgqr_") */
    std::string orgqr_sym(sym);
    auto pos = orgqr_sym.find("geqrf");
    if (pos != std::string::npos) orgqr_sym.replace(pos, 5, "orgqr");
    auto *fn_orgqr = reinterpret_cast<porgqr_fn_t>(try_load_sym(lib, orgqr_sym.c_str()));
    ...
    if (!fn_orgqr || !fn_gemm) {
        BlacsResult br = {false, -1.0, 0};
        const char *msg = !fn_orgqr ? "error=porgqr_not_found"
                                     : "error=pgemm_not_found";
        report_blacs_result("PGEQRF", msg, br, format);
        pc.finalize();
        return;
    }
    ...
    if (info != 0) {
        if (pc.bc.is_root()) {
            ...
            report_lapack_result("PGEQRF", params_str, lr, format);
        }
    ...
    /* Save R from the upper triangle of factored A.
       ...
       We need to copy it before PDORGQR overwrites A_loc with Q. */
    ...
    /* Call PDORGQR to get Q (m-by-mn) from the factored A_loc.
       Q overwrites A_loc (first mn columns). */
    int lwork_orgqr = -1, info_orgqr = 0;
    work_query = std::malloc(ctx.typesize);
    fn_orgqr(&m, &mn, &mn, A_loc, &one, &one, desc_a,
             tau, work_query, &lwork_orgqr, &info_orgqr);
    lwork_orgqr = query_lwork(work_query, ctx);
    if (lwork_orgqr < 1) lwork_orgqr = 1;
    std::free(work_query);

    work = std::malloc(static_cast<std::size_t>(lwork_orgqr) * ctx.typesize);
    info_orgqr = 0;
    fn_orgqr(&m, &mn, &mn, A_loc, &one, &one, desc_a,
             tau, work, &lwork_orgqr, &info_orgqr);
    std::free(work);
    std::free(tau);
    ...
    MpfrMatrix A_local_ref(loc_m, loc_n, prec);
    extract_local_mpfr(A_local_ref, A_orig, m, n, pc.mb, pc.nb,
                       pc.bc.myrow, pc.bc.mycol, pc.bc.nprow, pc.bc.npcol);

    ErrorResult err = compute_error_matrix(A_local_ref, C_loc, lld_c, ctx);
    ...
    if (pc.bc.is_root()) {
        ...
        lr.orthogonality = -1.0;  /* Would need Q^T*Q check */
        lr.info = (info != 0) ? info : info_orgqr;
        report_lapack_result("PGEQRF", params_str, lr, format);
    }
