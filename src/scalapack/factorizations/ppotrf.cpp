/* ppotrf.cpp -- ScaLAPACK PPOTRF accuracy tester
   (Cholesky factorization, verified via solve) */
...
void test_ppotrf(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PPOTRF", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PPOTRF", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (!fn_potrs) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PPOTRF", "error=ppotrs_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char uplo : {'L', 'U'}) {
        ...
        if (pc.bc.is_root()) {
            ...
            report_lapack_result("PPOTRF", params_str, lr, format);
        }
