/* cppotrf.cpp -- ScaLAPACK CPPOTRF accuracy tester
   (Complex Cholesky factorization of Hermitian positive definite matrix, verified via solve) */
...
extern "C" typedef void (*ppotrf_fn_t)(
    const char *uplo, const int *n, void *a,
...
void test_cppotrf(const TesterCtx &ctx, void *lib, const char *sym,
                  const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("CPPOTRF", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<ppotrf_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPPOTRF", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }

    /* Derive PPOTRS symbol from PPOTRF symbol */
    std::string potrs_sym(sym);
    auto pos = potrs_sym.find("potrf");
    if (pos != std::string::npos) potrs_sym.replace(pos, 5, "potrs");
    auto *fn_potrs = reinterpret_cast<ppotrs_fn_t>(try_load_sym(lib, potrs_sym.c_str()));
    if (!fn_potrs) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPPOTRF", "error=ppotrs_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char uplo : {'L', 'U'}) {
        ...
        /* Generate Hermitian positive definite matrix A(n x n) and complex RHS B(n x nrhs) */
        ...
        int info_potrs = 0;
        if (info == 0) {
            /* Call PPOTRS: solve A*X = B using Cholesky factor */
            fn_potrs(&uplo, &n, &nrhs,
                     A_loc, &one, &one, desc_a,
                     B_loc, &one, &one, desc_b,
                     &info_potrs, (std::size_t)1);
        }
        ...
        if (pc.bc.is_root()) {
            ...
            lr.info = (info != 0) ? info : info_potrs;
            report_lapack_result("CPPOTRF", params_str, lr, format);
        }
