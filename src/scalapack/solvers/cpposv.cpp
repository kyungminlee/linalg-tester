/* cpposv.cpp -- ScaLAPACK CPPOSV accuracy tester
   (Hermitian positive definite linear solve via Cholesky factorization) */
...
void test_cpposv(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("CPPOSV", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("CPPOSV", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    for (char uplo : {'L', 'U'}) {
        ...
        if (pc.bc.is_root()) {
            ...
            lr.info = info;
            report_lapack_result("CPPOSV", params_str, lr, format);
        }
