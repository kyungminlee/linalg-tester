/* pgesv.cpp -- ScaLAPACK PGESV accuracy tester
   (General linear solve via LU factorization) */
...
void test_pgesv(const TesterCtx &ctx, void *lib, const char *sym,
                const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PGESV", "error=init_failed", br, format);
        return;
    }
    ...
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PGESV", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (pc.bc.is_root()) {
        ...
        lr.info = info;
        report_lapack_result("PGESV", params_str, lr, format);
    }
