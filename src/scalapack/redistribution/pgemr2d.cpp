/* pgemr2d.cpp -- ScaLAPACK PDGEMR2D accuracy tester (matrix redistribution) */
...
void test_pgemr2d(const TesterCtx &ctx, void *lib, const char *sym,
                  const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb1, nb1)) {
        ...
        report_blacs_result("PGEMR2D", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<pgemr2d_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PGEMR2D", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Call PDGEMR2D: redistribute from (mb1, nb1) to (mb2, nb2) */
    fn(&m, &n,
       A_loc, &one, &one, descA,
       B_loc, &one, &one, descB,
       &pc.bc.ictxt);
    ...
    /* Compare B_loc vs B_ref element-by-element */
    /* Redistribution should be exact (bitwise), but compute max_relative for robustness */
    double max_rel = 0.0;
    if (loc_m2 > 0 && loc_n2 > 0) {
        MpfrMatrix ref_mpfr(loc_m2, loc_n2, prec);
        custom_to_mpfr_mat(ref_mpfr, B_ref, lld2, ctx);

        ErrorResult err = compute_error_matrix(ref_mpfr, B_loc, lld2, ctx);
    ...
    if (pc.bc.is_root()) {
        ...
        lr.info = 0;
        report_lapack_result("PGEMR2D", ps, lr, format);
    }
