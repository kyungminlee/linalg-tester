/* pheevd.cpp -- ScaLAPACK PZHEEVD accuracy tester
   (Hermitian eigenvalue, divide & conquer) */
...
void test_pheevd(const TesterCtx &ctx, void *lib, const char *sym,
                 const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PHEEVD", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<pheevd_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PHEEVD", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (!pgemm) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PHEEVD", "error=pgemm_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Generate Hermitian positive definite A */
    ...
    /* Eigenvalue array (real, replicated) */
    ...
    if (info != 0) {
        if (pc.bc.is_root()) {
            ...
            report_lapack_result("PHEEVD", ps, lr, format);
        }
    } else {
        ...
        /* Convert local C (AZ) and Z to MPFR complex */
