/* psyev.cpp -- ScaLAPACK PDSYEV accuracy tester (symmetric eigenvalue) */
...
void test_psyev(const TesterCtx &ctx, void *lib, const char *sym,
                const TestParams &params, const std::string &format)
{
    ...
    if (!pc.init(lib, mb, nb)) {
        ...
        report_blacs_result("PSYEV", "error=init_failed", br, format);
        return;
    }

    auto *fn = reinterpret_cast<psyev_fn_t>(try_load_sym(lib, sym));
    if (!fn) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PSYEV", "error=symbol_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    if (!pgemm) {
        BlacsResult br = {false, -1.0, 0};
        report_blacs_result("PSYEV", "error=pgemm_not_found", br, format);
        pc.finalize();
        return;
    }
    ...
    /* Generate symmetric positive definite A (ensures real eigenvalues, stability) */
    ...
    /* Eigenvalue array (replicated on all processes) */
    ...
    scatter_global_to_local(A_loc, lld, A_g, n,
                            ...
                            ctx.typesize);
    /* Keep a copy for PDGEMM verification */
    ...
    /* Allocate workspace and call PDSYEV */
    ...
    if (info != 0) {
        if (pc.bc.is_root()) {
            ...
            report_lapack_result("PSYEV", ps, lr, format);
        }
    } else {
        ...
        /* Convert local C (AZ) and Z to MPFR for residual computation */
