/* dotu.cpp -- BLAS Level 1 DOTU accuracy tester (complex-only) */
...
/* DOTU: result = sum_i x(i) * y(i)  (unconjugated dot product)
   Same ABI issue as DOTC for complex return values. */

/* ------------------------------------------------------------------ */
/* Public entry point                                                   */
/* ------------------------------------------------------------------ */

void test_dotu(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    if (!ctx.complex_mode) {
        std::fprintf(stderr, "DOTU requires --complex\n");
        return;
    }
    ...
    if (ctx.complex_return_abi == ComplexReturnABI::Hidden) {
        ...
        if (ctx.typesize == 8) {
            ...
            std::fprintf(stderr, "DOTU: register ABI unsupported for typesize %zu\n",
                         ctx.typesize);
    ...
    /* MPFR reference: dotu = sum_i x[i] * y[i] (no conjugation) */
    ...
    report_result("DOTU", params_str, err, format);
