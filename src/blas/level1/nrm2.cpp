/* nrm2.cpp -- BLAS Level 1 NRM2 accuracy tester */
...
/* NRM2 returns a scalar by value. */

/* ------------------------------------------------------------------ */
/* Public entry point                                                   */
/* ------------------------------------------------------------------ */

void test_nrm2(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    ...
    if (ctx.typesize == 4) {
        ...
        std::fprintf(stderr, "NRM2: unsupported typesize %zu, skipping\n",
                     ctx.typesize);
        std::free(x);
        std::free(result_buf);
        return;
    ...
    /* MPFR reference: nrm2 = sqrt(sum_i x[i]^2) */
    MpfrScalar nrm2_ref(prec);
    {
        MpfrScalar acc(prec), t(prec);
        mpfr_set_d(acc.get(), 0.0, MPFR_RNDN);
        for (int i = 0; i < n; ++i) {
            mpfr_mul(t.get(), x_mpfr.at(i, 0), x_mpfr.at(i, 0), MPFR_RNDN);
            mpfr_add(acc.get(), acc.get(), t.get(), MPFR_RNDN);
        }
        mpfr_sqrt(nrm2_ref.get(), acc.get(), MPFR_RNDN);
    }
    ...
    std::snprintf(params_str, sizeof(params_str),
                  "n=%d incx=%d", n, incx);
    report_result("NRM2", params_str, err, format);
