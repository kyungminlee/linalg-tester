               ...
               const TestParams &params, const std::string &format)
{
    ...
    for (char diag : {'N', 'U'}) {
        ...
        void *X_out = std::malloc(static_cast<std::size_t>(ldb) * n * ctx.typesize);
        std::memcpy(X_out, B_in, static_cast<std::size_t>(ldb) * n * ctx.typesize);
        ...
        report_result("TRSM", params_str, err, format);
