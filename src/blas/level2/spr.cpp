/* spr.cpp -- BLAS Level 2 SPR accuracy tester */
...
extern "C" typedef void (*spr_fn_t)(
    const char *uplo,
    const int  *n,
    const void *alpha,
...
/* ------------------------------------------------------------------ */
/* MPFR reference: AP_ref[idx] = alpha * x[i] * x[j] + AP_in[idx]    */
/*   for each (i,j) in the uplo triangle                               */
/* ------------------------------------------------------------------ */

static void mpfr_spr_ref(MpfrMatrix &ap_ref,
                           int n, char uplo,
                           mpfr_t alpha,
                           const MpfrMatrix &x,
    ...
    if (uplo == 'U') {
        for (int j = 0; j < n; ++j) {
            for (int i = 0; i <= j; ++i) {
                ...
                mpfr_mul(tmp.get(), alpha, tmp.get(), MPFR_RNDN);
                /* ap_ref[pos] = tmp + ap_in[pos] */
                mpfr_add(ap_ref.at(pos, 0), tmp.get(), ap_in.at(pos, 0), MPFR_RNDN);
                ++pos;
            }
        }
    } else {
...
void test_spr(const TesterCtx &ctx, void *lib, const char *sym,
              const TestParams &params, const std::string &format)
{
    auto *fn = reinterpret_cast<spr_fn_t>(load_sym(lib, sym));
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        /* Read AP_in as a flat vector of length packed_len */
        MpfrMatrix ap_in_mpfr(packed_len, 1, prec);
        {
            const char *p = static_cast<const char *>(AP_in);
            for (int i = 0; i < packed_len; ++i)
                ctx.to_mpfr(ap_in_mpfr.at(i, 0), p + static_cast<std::size_t>(i) * ctx.typesize);
        }

        MpfrMatrix ap_ref(packed_len, 1, prec);
        mpfr_spr_ref(ap_ref, n, uplo,
                      mpfr_alpha.get(), x_mpfr, ap_in_mpfr);

        /* Compare packed output as a vector with inc=1 */
        int inc_one = 1;
