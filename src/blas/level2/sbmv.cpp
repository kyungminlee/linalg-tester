/* sbmv.cpp -- BLAS Level 2 SBMV accuracy tester */
...
extern "C" typedef void (*sbmv_fn_t)(
    const char *uplo,
...
/* ------------------------------------------------------------------ */
/* Expand symmetric banded storage to full MpfrMatrix                  */
/* ------------------------------------------------------------------ */

static void expand_symband_to_full(MpfrMatrix &full,
...
void test_sbmv(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    auto *fn = reinterpret_cast<sbmv_fn_t>(load_sym(lib, sym));
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        void *AB    = gen_symmetric_band_array(n, k, uplo,
        ...
        expand_symband_to_full(A_full, AB, ldab, n, k, uplo, ctx);
