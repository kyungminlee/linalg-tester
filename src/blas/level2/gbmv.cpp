/* gbmv.cpp -- BLAS Level 2 GBMV accuracy tester */
...
/* ------------------------------------------------------------------ */
/* Expand general banded storage to full MpfrMatrix                    */
/* ------------------------------------------------------------------ */

static void expand_band_to_full(MpfrMatrix &full,
                                 ...
                                 const TesterCtx &ctx)
{
    ...
    for (int j = 0; j < n; ++j) {
        int i_lo = std::max(0, j - ku);
        int i_hi = std::min(m - 1, j + kl);
        for (int i = i_lo; i <= i_hi; ++i) {
            std::size_t idx = static_cast<std::size_t>(ku + i - j)
...
static void mpfr_gbmv_ref(MpfrMatrix &y_ref,
...
void test_gbmv(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    auto *fn = reinterpret_cast<gbmv_fn_t>(load_sym(lib, sym));
    ...
    for (char trans : {'N', 'T', 'C'}) {
        ...
        void *AB    = gen_band_array(m, n, kl, ku,
        ...
        fn(&trans, &m, &n, &kl, &ku,
           alpha, AB, &ldab, x, &incx, beta, y_out, &incy,
           (std::size_t)1);
        ...
        MpfrMatrix A_full(m, n, prec);
        expand_band_to_full(A_full, AB, ldab, m, n, kl, ku, ctx);
        ...
        mpfr_gbmv_ref(y_ref, trans_ref, m, n,
        ...
        std::snprintf(params_str, sizeof(params_str),
                      "trans=%c m=%d n=%d kl=%d ku=%d",
                      trans, m, n, kl, ku);
        report_result("GBMV", params_str, err, &sr, format);
