/* geru.cpp -- BLAS Level 2 GERU accuracy tester (complex-only) */
...
extern "C" typedef void (*geru_fn_t)(
...
static void mpfr_geru_ref(MpfrComplexMatrix &A_ref,
                            int m, int n,
...
void test_geru(const TesterCtx &ctx, void *lib, const char *sym,
               const TestParams &params, const std::string &format)
{
    if (!ctx.complex_mode) {
        std::fprintf(stderr, "GERU requires --complex\n");
        return;
    }

    auto *fn = reinterpret_cast<geru_fn_t>(load_sym(lib, sym));
    ...
    mpfr_geru_ref(A_ref, m, n, mpfr_alpha, x_mpfr, y_mpfr, A_in_mpfr);
    ...
    std::snprintf(params_str, sizeof(params_str),
                  ...
                  m, n, incx, incy);
    report_result("GERU", params_str, err, &sr, format);
