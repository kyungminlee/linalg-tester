/* syevd.cpp -- Mirror tester for LAPACK SYEVD (symmetric eigenvalues, D&C) */
...
void mirror_test_syevd(const MirrorSide &a, const MirrorSide &b,
                        const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        auto run_side = [&](const MirrorSide &s, MpfrMatrix &W_out) {
            ...
            auto *fn = reinterpret_cast<syevd_fn_t>(
        ...
        std::snprintf(params_str, sizeof(params_str),
                      "uplo=%c n=%d", uplo, n);
        mirror_report_result("SYEVD", params_str, err,
