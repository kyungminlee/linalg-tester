/* cgelqf.cpp -- Mirror tester for LAPACK complex GELQF (LQ factorization) */
...
extern "C" typedef void (*cgelqf_fn_t)(
    const int *m, const int *n, void *A, const int *lda,
    void *tau, void *work, const int *lwork, int *info);

void mirror_test_cgelqf(const MirrorSide &a, const MirrorSide &b,
                          const TestParams &params, const MirrorConfig &config)
{
    ...
    auto run_side = [&](const MirrorSide &side, MpfrComplexMatrix &result,
        ...
        auto *fn = reinterpret_cast<cgelqf_fn_t>(
        ...
        native_complex_array_to_mpfr(tau_out, tau, mn, side.ctx);
    ...
    MpfrComplexMatrix tau_a(mn, 1, prec);
    MpfrComplexMatrix tau_b(mn, 1, prec);
    ...
    const MpfrComplexMatrix &tau_ref = (config.reference == "a") ? tau_a : tau_b;
    const MpfrComplexMatrix &tau_tst = (config.reference == "a") ? tau_b : tau_a;
    ErrorResult tau_err = compute_error_mpfr_complex_vector(tau_ref, tau_tst, prec);

    char params_str[256];
    std::snprintf(params_str, sizeof(params_str),
                  "m=%d n=%d tau_maxerr=%.3e", m, n, tau_err.max_relative);
    mirror_report_result("CGELQF", params_str, err,
