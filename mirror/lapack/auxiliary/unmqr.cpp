/* unmqr.cpp -- Mirror tester for LAPACK UNMQR (multiply by unitary Q from QR) */
...
void mirror_test_unmqr(const MirrorSide &a, const MirrorSide &b,
                        const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char trans : {'N', 'C'}) {
        ...
        auto run_side = [&](const MirrorSide &side, MpfrComplexMatrix &result) {
            ...
            /* Materialize C */
            void *native_C = mpfr_complex_mat_to_native(C_mpfr, ldc, side.ctx);

            /* Workspace query for UNMQR */
            lwork_q = -1;
            work_q = std::malloc(side.ctx.typesize);
            info = 0;
            unmqr(&side_ch, &trans, &m, &nrhs, &k_qr,
                  native_A, &lda, tau, native_C, &ldc,
                  work_q, &lwork_q, &info, (std::size_t)1, (std::size_t)1);
            int lwork_unmqr = mirror_query_lwork_complex(work_q, side.ctx);
            std::free(work_q);

            /* Re-materialize C */
            std::free(native_C);
            native_C = mpfr_complex_mat_to_native(C_mpfr, ldc, side.ctx);

            /* Apply Q */
            void *work_unmqr = std::malloc(
                static_cast<std::size_t>(lwork_unmqr) * side.ctx.typesize);
            info = 0;
            unmqr(&side_ch, &trans, &m, &nrhs, &k_qr,
                  native_A, &lda, tau, native_C, &ldc,
                  work_unmqr, &lwork_unmqr, &info,
                  (std::size_t)1, (std::size_t)1);

            custom_to_mpfr_complex_mat(result, native_C, ldc, side.ctx);
        ...
        MpfrComplexMatrix res_a(m, nrhs, prec);
        MpfrComplexMatrix res_b(m, nrhs, prec);
        run_side(a, res_a);
        run_side(b, res_b);
        ...
        mirror_report_result("UNMQR", params_str, err,
