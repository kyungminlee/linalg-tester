/* clange.cpp -- Mirror tester for LAPACK complex LANGE (matrix norm) */
...
void mirror_test_clange(const MirrorSide &a, const MirrorSide &b,
                         const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char norm_type : {'M', '1', 'I', 'F'}) {
        ...
        auto run_side = [&](const MirrorSide &side, MpfrScalar &result) {
            ...
            /* work: real array of size max(1,m) for 'I' norm */
            std::size_t real_ts = side.ctx.typesize / 2;
            void *work = std::calloc(
                static_cast<std::size_t>(std::max(1, m)), real_ts);
        ...
        mirror_report_result("CLANGE", params_str, err,
