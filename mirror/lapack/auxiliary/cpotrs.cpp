/* cpotrs.cpp -- Mirror tester for LAPACK complex POTRS (Cholesky-based solve) */
...
extern "C" typedef void (*cpotrs_fn_t)(
    const char *uplo, const int *n, const int *nrhs,
    const void *A, const int *lda,
    void *B, const int *ldb, int *info, std::size_t);

void mirror_test_cpotrs(const MirrorSide &a, const MirrorSide &b,
                         const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char uplo : {'U', 'L'}) {
        ...
        auto run_side = [&](const MirrorSide &side, MpfrComplexMatrix &result) {
            std::string cpotrf_sym = replace_suffix(side.sym, "potrs", "potrf");
            auto *cpotrf = reinterpret_cast<cpotrf_fn_t>(
                load_sym(side.lib, cpotrf_sym.c_str()));
            auto *cpotrs = reinterpret_cast<cpotrs_fn_t>(
            ...
            info = 0;
            cpotrs(&uplo, &n, &nrhs, native_A, &lda, native_B, &ldb,
                   &info, (std::size_t)1);
        ...
        mirror_report_result("CPOTRS", params_str, err,
