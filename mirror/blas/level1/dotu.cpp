/* dotu.cpp -- Mirror tester for BLAS Level 1 DOTU (complex-only) */
                       ...
                       const TestParams &params, const MirrorConfig &config)
{
    ...
    auto run_side = [&](const MirrorSide &side, MpfrComplexScalar &result) {
        ...
        if (side.ctx.complex_return_abi == ComplexReturnABI::Hidden) {
            ...
            if (side.ctx.typesize == 8) {
                ...
                std::fprintf(stderr,
                    "DOTU mirror: register ABI unsupported for typesize %zu\n",
                    side.ctx.typesize);
    ...
    mirror_report_result("DOTU", params_str, err, nullptr, nullptr, config);
}
