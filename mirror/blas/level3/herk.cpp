/* herk.cpp -- Mirror tester for BLAS Level 3 HERK (complex-only) */
...
/* Helper: materialize an MpfrScalar as a real native value.
   ...
   but alpha/beta for HERK are real scalars of size typesize/2. */
                       ...
                       const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char trans : {'N', 'C'}) {
        ...
        auto run_side = [&](const MirrorSide &s, MpfrComplexMatrix &result) {
            ...
            void *native_alpha = mpfr_real_scalar_to_native(alpha_mpfr, s.ctx);
            void *native_beta  = mpfr_real_scalar_to_native(beta_mpfr, s.ctx);

            auto *fn = reinterpret_cast<herk_fn_t>(
        ...
        mirror_report_result("HERK", params_str, err,
