/* ptranu.cpp -- Mirror tester for PBLAS Level 3 PTRANU (complex unconjugated transpose) */
...
void mirror_test_ptranu(const MirrorSide &a, const MirrorSide &b,
                          const TestParams &params, const MirrorConfig &config)
{
    ...
    /* A is m x n, C is n x m.  C = alpha * A^T + beta * C (no conjugate) */
    ...
    /* Use alpha=1, beta=0 for a pure unconjugated transpose */
    ...
    auto run_side = [&](const MirrorSide &s, MpfrComplexMatrix &result) {
        ...
        int one = 1;
        auto *fn = reinterpret_cast<ptranu_fn_t>(
    ...
    mirror_report_result("PTRANU", ps, err, nullptr, nullptr, config);
}
