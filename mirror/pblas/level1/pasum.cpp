/* pasum.cpp -- Mirror tester for PBLAS Level 1 PASUM (absolute sum) */
...
void mirror_test_pasum(const MirrorSide &a, const MirrorSide &b,
                         const TestParams &params, const MirrorConfig &config)
{
    ...
    auto run_side = [&](const MirrorSide &s, MpfrScalar &result) {
        ...
        void *asum_buf = std::calloc(1, s.ctx.typesize);

        int desc_x[9];
        mpc.make_desc(desc_x, n, 1, lld_x);

        int one = 1;
        auto *fn = reinterpret_cast<pasum_fn_t>(
            load_sym(s.lib, s.sym.c_str()));
        fn(&n, asum_buf, x_loc, &one, &one, desc_x, &one);

        s.ctx.to_mpfr(result.get(), asum_buf);
    ...
    mirror_report_result("PASUM", ps, err, nullptr, nullptr, config);
}
