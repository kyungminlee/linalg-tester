/* ptrsv.cpp -- Mirror tester for PBLAS Level 2 PTRSV */
...
void mirror_test_ptrsv(const MirrorSide &a, const MirrorSide &b,
                         const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char diag : {'N', 'U'}) {
        ...
        auto run_side = [&](const MirrorSide &s, MpfrMatrix &result) {
            ...
            int one = 1;
            auto *fn = reinterpret_cast<ptrsv_fn_t>(
        ...
        std::snprintf(ps, sizeof(ps), "uplo=%c trans=%c diag=%c n=%d",
                      uplo, trans, diag, n);
        mirror_report_result("PTRSV", ps, err, nullptr, nullptr, config);
    }}}
}
