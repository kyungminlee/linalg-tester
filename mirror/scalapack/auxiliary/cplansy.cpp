/* cplansy.cpp -- Mirror tester for ScaLAPACK complex PLANSY (distributed complex symmetric norm) */

#include "../../pblas/mirror_pblas_common.h"
#include "../../lapack/mirror_lapack_common.h"

extern "C" typedef float (*cplansy_s_fn_t)(
...
extern "C" typedef double (*cplansy_d_fn_t)(
    const char *norm, const char *uplo, const int *n,
    const void *A, const int *ia, const int *ja, const int *desca,
    void *work, std::size_t, std::size_t);

extern "C" typedef long double (*cplansy_l_fn_t)(
    const char *norm, const char *uplo, const int *n,
    const void *A, const int *ia, const int *ja, const int *desca,
    void *work, std::size_t, std::size_t);

void mirror_test_cplansy(const MirrorSide &a, const MirrorSide &b,
    ...
    for (char norm : {'M', '1', 'I', 'F'}) {
        for (char uplo : {'U', 'L'}) {
            ...
            auto run_side = [&](const MirrorSide &side, MpfrScalar &result) {
                ...
                /* Work buffer uses real element size */
                std::size_t real_ts = side.ctx.typesize / 2;
                int work_size = std::max(loc_rA, loc_cA) + 1;
                void *work = std::calloc(work_size, real_ts);

                int one = 1;

                if (real_ts == 4) {
                    auto *fn = reinterpret_cast<cplansy_s_fn_t>(
                    ...
                    auto *fn = reinterpret_cast<cplansy_d_fn_t>(
                        load_sym(side.lib, side.sym.c_str()));
                    ...
                    auto *fn = reinterpret_cast<cplansy_l_fn_t>(
                        load_sym(side.lib, side.sym.c_str()));
            ...
            mirror_report_result("CPLANSY", ps, err, nullptr, nullptr, config);
        }
    }
}
