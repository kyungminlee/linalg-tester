/* planhe.cpp -- Mirror tester for ScaLAPACK PLANHE (distributed Hermitian matrix norm) */

#include "../../pblas/mirror_pblas_common.h"
#include "../../lapack/mirror_lapack_common.h"

extern "C" typedef float (*planhe_s_fn_t)(
...
extern "C" typedef double (*planhe_d_fn_t)(
    const char *norm, const char *uplo, const int *n,
    const void *A, const int *ia, const int *ja, const int *desca,
    void *work, std::size_t, std::size_t);

extern "C" typedef long double (*planhe_l_fn_t)(
    const char *norm, const char *uplo, const int *n,
    const void *A, const int *ia, const int *ja, const int *desca,
    void *work, std::size_t, std::size_t);

void mirror_test_planhe(const MirrorSide &a, const MirrorSide &b,
    ...
    for (char norm : {'M', '1', 'I', 'F'}) {
        for (char uplo : {'U', 'L'}) {
            ...
            auto run_side = [&](const MirrorSide &side, MpfrScalar &result) {
                ...
                if (real_ts == 4) {
                    auto *fn = reinterpret_cast<planhe_s_fn_t>(
                    ...
                    auto *fn = reinterpret_cast<planhe_d_fn_t>(
                        load_sym(side.lib, side.sym.c_str()));
                    ...
                    auto *fn = reinterpret_cast<planhe_l_fn_t>(
                        load_sym(side.lib, side.sym.c_str()));
            ...
            mirror_report_result("PLANHE", ps, err, nullptr, nullptr, config);
        }
    }
}
