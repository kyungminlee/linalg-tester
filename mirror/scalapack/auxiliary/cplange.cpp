/* cplange.cpp -- Mirror tester for ScaLAPACK complex PLANGE (distributed matrix norm) */
...
void mirror_test_cplange(const MirrorSide &a, const MirrorSide &b,
                          const TestParams &params, const MirrorConfig &config)
{
    ...
    for (char norm : {'M', '1', 'I', 'F'}) {
        auto run_side = [&](const MirrorSide &side, MpfrScalar &result) {
            ...
            /* Return type is real, dispatch by real typesize */
            if (real_ts == 4) {
        ...
        mirror_report_result("CPLANGE", ps, err, nullptr, nullptr, config);
    }
}
