/* blacs_gamx2d.cpp -- Mirror tester for BLACS global max */
/* Compares xGAMX2D across two library implementations.
   On single process: result = input (max of 1 element).
...
/* Fortran ABI for xGAMX2D */
extern "C" typedef void (*gamx2d_fn_t)(
...
void mirror_test_blacs_gamx2d(const MirrorSide &a, const MirrorSide &b,
                               const TestParams &params, const MirrorConfig &config)
{
    ...
    auto run_side = [&](const MirrorSide &side, MpfrMatrix &result) {
        ...
        BlacsCtx bc;
        if (!bc.load(side.lib)) {
            std::fprintf(stderr, "mirror_blacs_gamx2d: failed to load BLACS symbols from side %s\n",
                         side.label.c_str());
            return;
        }

        /* Load gamx2d symbol */
        auto *fn_gamx = reinterpret_cast<gamx2d_fn_t>(
        ...
        /* Call GAMX2D -- modifies A in place, ra/ca = nullptr (ldia = -1) */
        fn_gamx(&bc.ictxt, &scope, &top, &m, &n, native_A, &lda,
    ...
    std::snprintf(params_str, sizeof(params_str),
                  "m=%d n=%d scope=A", m, n);
    mirror_report_result("BLACS_GAMX2D", params_str, err, nullptr, nullptr, config);
}
