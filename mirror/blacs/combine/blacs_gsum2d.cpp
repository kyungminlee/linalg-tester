/* blacs_gsum2d.cpp -- Mirror tester for BLACS global sum */
/* Compares xGSUM2D across two library implementations.
   On single process: result = input (sum of 1 element).
...
/* Fortran ABI for xGSUM2D */
extern "C" typedef void (*gsum2d_fn_t)(
...
void mirror_test_blacs_gsum2d(const MirrorSide &a, const MirrorSide &b,
                               const TestParams &params, const MirrorConfig &config)
{
    ...
    auto run_side = [&](const MirrorSide &side, MpfrMatrix &result) {
        ...
        BlacsCtx bc;
        if (!bc.load(side.lib)) {
            std::fprintf(stderr, "mirror_blacs_gsum2d: failed to load BLACS symbols from side %s\n",
                         side.label.c_str());
            return;
        }

        /* Load gsum2d symbol */
        auto *fn_gsum = reinterpret_cast<gsum2d_fn_t>(
        ...
        /* Call GSUM2D -- modifies A in place */
        fn_gsum(&bc.ictxt, &scope, &top, &m, &n, native_A, &lda,
    ...
    std::snprintf(params_str, sizeof(params_str),
                  "m=%d n=%d scope=A", m, n);
    mirror_report_result("BLACS_GSUM2D", params_str, err, nullptr, nullptr, config);
}
