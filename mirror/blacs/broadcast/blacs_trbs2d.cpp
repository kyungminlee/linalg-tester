/* blacs_trbs2d.cpp -- Mirror tester for BLACS triangular broadcast */
/* Compares xTRBS2D/xTRBR2D across two library implementations.
...
/* Fortran ABI for xTRBS2D (triangular broadcast send) */
extern "C" typedef void (*trbs2d_fn_t)(
...
/* Fortran ABI for xTRBR2D (triangular broadcast receive) */
extern "C" typedef void (*trbr2d_fn_t)(
    const int *ictxt, const char *scope, const char *top,
    const char *uplo, const char *diag,
...
void mirror_test_blacs_trbs2d(const MirrorSide &a, const MirrorSide &b,
    ...
    for (char uplo : {'U', 'L'}) {
        for (char diag : {'N', 'U'}) {
            ...
            auto run_side = [&](const MirrorSide &side, MpfrMatrix &result) {
                ...
                BlacsCtx bc;
                if (!bc.load(side.lib)) {
                    std::fprintf(stderr, "mirror_blacs_trbs2d: failed to load BLACS symbols from side %s\n",
                                 side.label.c_str());
                    return;
                }

                /* Load broadcast send symbol */
                auto *fn_bsend = reinterpret_cast<trbs2d_fn_t>(
                    load_sym(side.lib, side.sym.c_str()));

                /* Derive receive symbol: replace "trbs2d" with "trbr2d" */
                std::string recv_sym(side.sym);
                auto pos = recv_sym.find("trbs2d");
                if (pos != std::string::npos)
                    recv_sym.replace(pos, 6, "trbr2d");
                auto *fn_brecv = reinterpret_cast<trbr2d_fn_t>(
                ...
                /* Root broadcasts */
                fn_bsend(&bc.ictxt, &scope, &top, &uplo, &diag,
                         &m, &n, native_A, &lda,
                ...
                int rsrc = 0, csrc = 0;
                fn_brecv(&bc.ictxt, &scope, &top, &uplo, &diag,
                         &m, &n, native_B, &lda, &rsrc, &csrc,
                         (std::size_t)1, (std::size_t)1,
                         (std::size_t)1, (std::size_t)1);
            ...
            std::snprintf(params_str, sizeof(params_str),
                          "uplo=%c diag=%c m=%d n=%d scope=A", uplo, diag, m, n);
            mirror_report_result("BLACS_TRBS2D", params_str, err,
